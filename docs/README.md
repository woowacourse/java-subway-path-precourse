...
- [ ] 가중치를 저장해두는 Repository를 만든다.
  - [ ] 각 가중치는 거리, 시간을 저장한 Map을 변수로 갖는다.
  - [ ] 두 역을 입력받아 거리, 시간 가중치를 반환하는 기능을 만든다.
- [ ] 최단경로를 저장해두는 Repository를 만든다.
  - [ ] 거리 기준, 시간 기준 경로를 각각 만들어둔다.
  - [ ] 가중치 기준, 출발역과 도착역을 입력하면 해당 최단경로를 반환하는 기능을 만든다.
