...
- [x] 메인 화면
  - [x] 1 입력시 경로 조회로 넘어간다.
  - [x] Q 입력시 프로그램을 종료한다.
  - [x] 이외의 문자 입력시 에러 메시지와 함께 재입력을 받는다.

- [ ] 경로 조회
  - [x] 경로 기준을 선택한다.
    - [x] 1 입력시 최단 거리를 조회
       - [ ] 최단 거리로 갔을 때, 총 소요 시간도 같이 출력한다.
    - [x] 2 입력시 최소 시간를 조회
       - [ ] 최소 시간으로 갈 때, 총 거리도 같이 출력한다.
    - [x] B 입력시 메인 화면으로 돌아간다.
    - [x] 이외의 문자 입력시 에러 메시지와 함께 재입력을 받는다.
    - [x] 경로 조회 시 출발역과 도착역이 같으면 에러를 출력한다.
    - [x] 경로 조회 시 출발역과 도착역이 연결되어 있지 않으면 에러를 출력한다.
    - [x] 그 외 정상적으로 프로그램이 수행되지 않은 경우 에러를 출력한다.
    - [x] 마지막 글자가 '역'으로 끝나지 않으면 재입력을 받는다.
    
- [x] 조회 결과
  - [x] 사용자가 선택한 조회 결과를 출력한다.
  - [x] 메인 화면으로 복귀한다.
