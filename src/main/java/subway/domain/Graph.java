...
public class Graph {
	...
	public static void addTimeEdge(String firstStationName, String secondStationName,
		int timeWeight) {
		TIME_GRAPH
			.setEdgeWeight(TIME_GRAPH.addEdge(firstStationName, secondStationName), timeWeight);
	}

	public static void addDistanceEdge(String firstStationName, String secondStationName,
		int distanceWeight) {
		DISTANCE_GRAPH.setEdgeWeight(DISTANCE_GRAPH.addEdge(firstStationName, secondStationName),
			distanceWeight);
	}
	...
	public static int getDistanceByVertexList(List<String> stringVertexList) {
		...
		for (int index = 0; index < end_index; index++) {
			totalDistance += (int) DISTANCE_GRAPH.getEdgeWeight(
				DISTANCE_GRAPH
					.getEdge(stringVertexList.get(index), stringVertexList.get(index + 1)));
		}
