...
- SubwayRun ( 지하철 관련 기능을 실행하는 클래스 )

- subwaymanager ( 패키지 생성 )
    - MainSelectManager ( 시작 메인 화면 선택 클래스 )
    - RouteLookupManager ( 경로 조회 클래스 )
    
- utils ( 패키지 생성 )
    - OutputView ( 출력을 담당하는 클래스 )
        1. 메인 목차 출력
        2. 경로 조회 목차 출력
        3. 최단 거리 결과 출력 [INFO]를 붙여서 출력
        4. 최단 시간 결과 출력 [INFO]를 붙여서 출력
    - InputView ( 입력을 담당하는 클래스 )
    - Constant ( 상수 집합 클래스 )

- validator ( 유효성 검사 패키지)
    - Validator (유효성 검사 담당 클래스)
        1. 경로 조회 시 출발역과 도착역이 같으면 에러 출력 ( [ERROR] 붙여서 출력 / 최단 거리와 최소 시간 둘 다 포함 )
        2. 경로 조회 시 출발역과 도착역이 연결되어 있지 않으면 에러 출력 ( [ERROR] 붙여서 출력 / 최단 거리와 최소 시간 둘 다 포함 )
        3. 그 외 정상적 프로그램이 수행되지 않은 경우 에러 출력 ( [ERROR] 붙여서 출력 )
