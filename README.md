...
## 구현할 기능 목록
- 역과 역 사이의 `거리`(km)와 `소요 시간`(분) 정보들을 담는 구간 클래스 구현

- 초기 설정 기능
```
- 지하철역: 교대역, 강남역, 역삼역, 남부터미널역, 양재역, 양재시민의숲역, 매봉역
  왼쪽 끝이 상행 종점, 단위 ( km / 분 )
- 2호선: 교대역 - ( 2 / 3 ) - 강남역 - ( 2 / 3 ) - 역삼역
- 3호선: 교대역 - ( 3 / 2 ) - 남부터미널역 - ( 6 / 5 ) - 양재역 - ( 1 / 1 ) - 매봉역
- 신당선: 강남역 - ( 2 / 8 ) - 양재역 - ( 10 / 3 ) - 양재시민의숲역
 ```

- `최단 거리` 기준 경로 찾기 기능
- `최소 시간` 기준 경로 찾기 기능
- 경로의 총 거리, 총 소요 시간 출력 기능

- 입력값으로 경로 기준(거리 || 시간)을 선택하는 기능

- 경로 조회 시 출발역과 도착역이 같은지 확인하는 기능
- 경로 조회 시 출발역과 도착역이 연결되어 있는지 확인하는 기능
