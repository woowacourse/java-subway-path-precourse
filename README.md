...
- **초기화**
  ...
  - 지하철 노선 : 2호선 (교대역, 강남역, 역삼역), 3호선 (교대역, 남부터미널역, 양재역, 매봉역), 신분당선 (강남역, 양재역, 양재시민의숲역)
- **메인 화면**
  - 경로 조회(1) 또는 종료(Q)를 입력받는다
  - 입력에 따라 경로 조회를 진행 또는 실행을 종료시킨다.
  - [ERROR] 입력으로 1, Q 이외의 값이 들어올 경우 에러를 출력한다
- **경로 조회**
  - 사용자 입력
    - 최단 거리(1), 최소 시간(2), 돌아가기(B) 세 가지 선택지 중 하나를 입력받는다
    - 출발역과 도착역을 입력받는다
    - [ERROR] 입력으로 1, 2, B 이외의 값이 들어올 경우 에러를 출력한다
    - [ERROR] 출발역과 도착역이 같으면 에러를 출력한다
    - [ERROR] 출발역과 도착역이 연결되어 있지 않으면 에러를 출력한다
  - 경로 출력
    - 입력받은 출발역과 도착역에 따른 경로를 구한다
    - 해당 경로에 따른 총 소요 시간을 구한다
    - 해당 경로에 따른 총 거리를 구한다
    - 형식에 맞게 결과를 출력한다
    - [ERROR] 출발역과 도착역이 연결되어 있지 않으면 에러를 출력한다
...
- **view**
  - Input : 사용자에게 상황에 따라 필요한 입력을 받는 함수들이 구현된 클래스
  - Output : 사용자들에게 결과를 출력하는 함수들이 구현된 클래스
- **domain**
  - Station : 역 이름을 멤버로 가지는 지하철 역 객체 클래스
  - StationRepository : 초기화를 수행하며, 현재 존재하는 역의 리스트를 가지고 관리하는 클래스
  - Line : 노선 이름과 노선에 포함된 역 리스트를 멤버로 가지는 지하철 노선 객체 클래스
  - LineRespository : 초기화를 수행하며, 현재 존재하는 노선의 리스트를 가지고 관리하는 클래스
  - Path : 경로에 포함된 역 리스트와 총 소요시간, 총 거리를 멤버로 가지는 경로 객체 클래스
  - PathRepository : 초기화를 수행하며, 최소 경로와 최단 거리 정보를 관리하는 그래프를 가지고 관리하는 클래스
  - Constant : 전체 실행 과정에서 자주 사용되는 상수가 정의된 클래스
- **controller**
  - Controller : 전체 과정을 관리하고 각 클래스의 함수를 호출해 실행하는 클래스
