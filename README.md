...
- [x] 프로그램 시작 시, 역, 노선, 구간 정보를 초기 설정한다.  
    - [x] 초기화 시에는 역, 노선 정보를 Line과 Station에 설정하고  
    - [x] 구간 정보는 별도의 클래스를 생성하여 jgrapht를 이용해 시간과 거리로 나눠 저장한다.  
- [x] 메인 화면을 출력한다. (UI)  
...
- [x] 사용자에게 경로 조회 기능 목록 중 사용할 기능을 입력받는다. (UI)  
    - [x] (예외) 입력이 1, 2, B 가 아니면 에러를 발생시킨다.  
- [x] 경로 조회 기능 중 사용자에게 입력받는 기능 구현 (UI)  
    - [x] (예외) 출발역과 도착역이 동일한 경우 예외 처리   
    - [x] (예외) 출발역과 도착역이 연결되어 있지 않으면 예외 처리  
- [x] 경로 조회 결과 출력 기능 구현 (UI)  
    - [x] 경로 조회 결과 출력 시 최단거리로 한 경우, 최단 거리 및 시간 출력하기  
    - [x] 경로 조회 결과 출력 시 최소시간으로 한 경우, 최소 시간 및 거리 출력하기  
