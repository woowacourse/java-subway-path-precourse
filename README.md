...
```
 ...
 ```
### 메인 화면 기능
- '1'을 선택하면 경로 조회 세부화면으로 이동할 수 있다.
- 'Q'를 선택하면 프로그램을 종료할 수 있다.
- 이외의 문자를 입력하면 에러를 출력해야 한다.
...
####2. 노선을 추가한다.
- 노선의 이름을 받아서 새로운 노선을 추가할 수 있다.
- 상행 종점역과 하행 종점역을 받아서 추가한다.
- 이미 존재하는 노선은 추가할 수 없다.
- 노선의 이름은 '선'으로 끝나야 한다.

####3. 구간을 추가한다.
다음의 세 가지를 입력받는다.
- 구간에 추가할 역 이름
- 추가될 위치의 이전 역에서 추가할 역 까지의 거리와 소요 시간
- 추가할 역에서 추가될 위치의 다음 역 까지의 거리와 소요 시간
...
- 출발역과 도착역은 같은 이름이어서는 안 된다.
- 출발역과 도착역은 역에 등록된 역이어야 한다.
- 출발역에서 도착역까지의 구간(환승 포함 가능)이 존재해야 한다.

### 최단 거리 기준 조회 기능
> 최단 거리 기준으로 경로를 조회한다.
>
####1. <Station, "거리"> 기준으로 그래프를 만든다.
- 모든 station을 vertex에 추가한다(역 조회 기능 이용).
- 모든 구간을 조회하면서 가중치가 "거리"인 edge를 추가한다.

####2. 출발역-도착역의 최단 거리를 구한다.
- 출발역-도착역으로 이동 가능한 구간이 없을 경우 에러를 출력한다.
- 최단 거리로 이동하는 역의 목록을 저장한다.

####3. 최단 거리로 이동하는 역의 소요 시간을 구한다.
- 구간 조회 기능을 이용하여 특정 역에서 특정 역으로 이동하는 시간을 합산한다.

### 최소 시간 기준 조회 기능
> 최단 거리 기준으로 경로를 조회한다.
>
####1. <Station, "소요 시간"> 기준으로 그래프를 만든다.
- 모든 station을 vertex에 추가한다(역 조회 기능 이용).
- 모든 구간을 조회하면서 가중치가 "시간"인 edge를 추가한다.

####2. 출발역-도착역의 최소 시간을 구한다.
- 출발역-도착역으로 이동 가능한 구간이 없을 경우 에러를 출력한다.
- 최단 거리로 이동하는 역의 목록을 저장한다.

####3. 최단 거리로 이동하는 역의 거리을 구한다.
- 구간 조회 기능을 이용하여 특정 역에서 특정 역으로 이동하는 시간을 합산한다.

###조회 결과 출력 기능
- 총 거리를 출력한다.
- 소요 시간을 출력한다.
- 최단 거리/최소 시간으로 조회한 구간 내의 모든 역을 출력한다.

## ✖ 예외 목록
|발생 기능|예외 상황|
|---|------------|
|출발/도착 역 입력|입력된 출발역 혹은 종착역이 존재하지 않는 역일 때|
|출발/도착 역 입력|입력된 출발역과 도착역이 같을 때|
|경로 조회|입력된 출발역에서 도착역으로 이동하는 경로가 없을 때|
