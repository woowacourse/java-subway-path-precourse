...
## 구현할 기능정리
- [ ] 주어진 정보로 역, 노선, 구간 정보를 초기설정 한다.
### 도메인
- [x] 노선은 구간들을 가진다.
- [x] 구간은 '거리' 와 '도착 시간' 을 가진다. (단위는 km, 분)
    - [x] 예외. 구간거리와 도착시간은 양의 정수여야 한다.
    
- [ ] 경로 조회 기능
    - [ ] 출발역과 도착역을 입력받아 경로를 조회한다.
        - [ ] 예외. 출역과 도착역이 같으면 예외가 발생한다.
        - [ ] 예외. 출발역과 도착역이 연결되어 있지 않으면 예외가 발생한다.
    - [ ] 경로 조회 기준은 '최단 거리', '최소 시간' 하나를 선택할 수 있다.
        - [x] 조회 기준은 jgrapht 라이브러리를 사용하여 구한다.
            - [x] jgrapht라이브러리에서 지하철역은 vertex로, 구간은 edge로 추가한다.
            - [x] 최단 거리 일때는 edge의 가중치를 거리로, 최소 시간일 때는 가중치를 시간으로 코딩한다.
    - [x] 경로 조회 시 총 거리, 총 시간을 출력하고, 구간에 있는 역들을 출력한다.
    
### 메뉴
- [x] 메인화면에서 경로 조회, 종료 메뉴를 출력한다.
- [x] 메뉴를 입력받으면 해당 메뉴를 실행한다.
    - [x] 예외. 경로 조회나 종료의 명령어가 아닌 값을 입력받으면 예외가 발생한다. 
- [ ] 경로 조회 시 경로 기준을 선택하는 메뉴를 출력한다.
    - [ ] 예외. 경로 조회나 종료의 명령어가 아닌 값을 입력받으면 예외가 발생한다.
    - [ ] 돌아가기를 누르면 메인메뉴가 다시 실행된다. 
    
### 예외처리
- [x] 프로그램이 정상적으로 작동하지 않으면 예외를 출력한다.
    - [x] 예외 메세지는 "[ERROR]" 로 시작해야한다.
