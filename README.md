...
| 기능 종류   | 기본 기능                              | 상세                                      |
| ----------- | -------------------------------------- | ----------------------------------------- |
| 사용자 입력 | 출발역, 도착역을 입력받는다.           | 사용자 입력 받는 함수를 만든다.           |
|             |                                        | -> 입력 받은 값의 동일여부를 검증한다.    |
|             | ui 입력값을 받는다.                    | 메인> 경로 조회, 종료                     |
|             |                                        | 경로 조회> 최단 거리, 최소 시간, 돌아가기 |
| 시스템 출력 | 기본 진행 메시지를 출력한다.           | 메인 화면, 기능 선택                      |
|             | 사용자 입력 받을 때 메시지를 출력한다. | 출발역, 도착역 입력                       |
|             | 에러 메시지를 출력한다.                |                                           |
...
| 노선      | 노선을 등록하거나 삭제한다.  |                                                        |
| 역        | 역을 등록하거나 삭제한다.    |                                                        |
| 구간      | 구간을 생성한다.             | 하나의 구간은 인접한 역과 가중치(거리, 시간)을 가진다. |
|           |                              | 역 == 정점, 구간 == 간선                               |
| 경로 조회 | 최단 거리 기준으로 조회한다. | 최단 거리 기준 조회한 역 정보를 출력한다.              |
|           |                              | 총 소요된 거리와 시간을 계산한다.                      |
|           | 최소 시간 기준으로 조회한다. | 최소 시간 기준 조회한 역 정보를 출력한다.              |
|           |                              | 총 소요된 거리와 시간을 계산한다.                      |
| 초기 설정 | 초기 정보를 설정한다.        | 필요한 역과 노선, 구간을 생성한다.                     |

### Controller

| 기능 종류 | 기본 기능                                | 상세                                               |
| --------- | ---------------------------------------- | -------------------------------------------------- |
| 예외 처리 | 사용자 입력에 대한 에러 문구를 출력한다. | 출발역과 도착역이 같을 때                          |
|           |                                          | 출발역과 도착역이 연결되어 있지 않을 때            |
|           |                                          | 옵션값이 잘못되었을 때                             |
|           |                                          | 등록되지 않은 역을 입력했을 때                     |
| 메인 화면 | 옵션 입력값에 따라 기능을 전환한다.      | 1. 경로 조회 Q. 종료                               |
| 경로 조회 | 옵션 입력값에 따라 기능을 전환한다.      | 1. 최단 거리 2. 최소 시간 B. 돌아가기              |
|           | 경로 기준에 따라 기능을 전환한다.        | 출발역, 도착역 사용자 입력을 받는 함수를 실행한다. |
|           |                                          | 입력값을 확인하고 예외 처리한다.                   |
|           |                                          | 경로 조회를 수행한다.                              |
| 초기 설정 | 초기 정보를 설정한다.                    |                                                    |
