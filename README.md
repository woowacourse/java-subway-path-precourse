...
```
 //초기 값 private 배열로 만들어 나중에 수정하기 편하게 만들기
...
### 경로 조회 기능
- 출발역과 도착역을 입력받아 경로를 조회한다. //출발역과 도착역 입력받기
- 경로 조회 시 총 거리, 총 소요 시간도 함께 출력한다. // 구간 검색후 구간 정보값 전부 더하기
- 경로 조회 기준은 `최단 거리` `최소 시간`이 있다. //최소값 찾기

### 예외 처리
- 경로 조회 시 출발역과 도착역이 같으면 에러를 출력한다. // 입력받을때 바로 찾기
- 경로 조회 시 출발역과 도착역이 연결되어 있지 않으면 에러를 출력한다. // 검색할때 찾기
- 그 외 정상적으로 프로그램이 수행되지 않은 경우 에러를 출력한다. // 에러값 생각해 보자
...
## ❗️힌트
### 최단 경로 라이브러리
- jgrapht 라이브러리를 활용하면 간편하게 최단거리를 조회할 수 있음  // jgrapht 검색해보기
- Dijkstra 알고리즘을 반드시 이해할 필요는 없고 미션에 적용할 정도로만 이해하면 됨 // Dijkstra 검색해보기
- JGraphtTest 클래스의 테스트를 활용하여 미션에 필요한 라이브러리의 기능을 학습할 수 있음  // 검색해본 정보로 JGraphtTest 사용해보기
