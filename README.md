...
- 사전 등록 정보로 지하철 역 초기 생성
  - 지하철 역 생성 기능
    - 중복된 지하철 역 이름은 등록 될 수 없다.
    - 등록 되지 않은 역을 찾을 시 에러를 출력 한다.
  - 지하철 노선 생성 기능
    - 지하철 노선은 2정거장 이상 가져야한다.
    - 지하철 노선에서 중복된 역은 포함 될 수 없다.
      - 지하철 노선은 생성 시 상행 종점역과 하행 종점역이 같을 수 없다.
      - 지하철 노선은 역 추가 시 중복된 역을 포함 할 수 없다.
    - 지하철 노선은 저장소에 등록 된 역만 추가 가능하다.
  - 구간 정보 생성 기능
    - 거리와 소요시간은 양의 정수이다.
- 경로 구하기 기능
  - 최단 거리 경로 구하기
  - 최소 시간 경로 구하기
  - 예외) 경로 조회 시 출발역과 도착역이 같으면 에러 출력
  - 예외) 경로 조회 시 출발역과 도착역이 연결되어 있지 않으면 에러 출력
  - 예외) 입력한 역이 등록되어 있지 않을 경우 에러 출력
- 총 거리 및 총 소요 시간 계산 기능
